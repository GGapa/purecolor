...
<head>
    ...
    <meta name="description" content="纯·色 — 简单轻量的在线色板工具，分享和发现精美配色方案">
    <meta name="theme-color" content="#5e72e4">
    <title>纯 · 色 — Pure Colors</title>
    <?php
        $res_prefix = "";
    ...
    <?php
        ...
        if (!isset($_GET['id']) || empty($_GET['id'])){
            ...
            if (!check_id($_GET['id'])){
                ...
                $stmt = mysqli_prepare($conn, "SELECT * FROM palettes WHERE id = ?");
...
<body>
    <div id="app">
        <!-- ===== Material App Bar ===== -->
        <header class="pc-app-bar" role="banner">
            <span class="material-symbols-outlined pc-app-bar-icon" aria-hidden="true">palette</span>
            <span class="pc-app-bar-title">纯·色</span>
            <span class="pc-app-bar-subtitle">Pure Colors</span>
            <span class="pc-app-bar-spacer"></span>
            <a class="pc-app-bar-github" href="https://github.com/GGapa/purecolor" target="_blank" rel="noopener" aria-label="GitHub 仓库">
        ...
        <!-- ===== Sidebar ===== -->
        <aside class="pc-sidebar" v-if="notfound == false" role="complementary" aria-label="颜色信息">
            <div class="pc-color-card">
                <div class="pc-color-preview" :style="'background-color: ' + color" aria-hidden="true"></div>
                <div class="pc-color-hex" role="button" tabindex="0" @click="copyToClipboard(color_uppercase)" @keydown.enter="copyToClipboard(color_uppercase)" :title="'点击复制 ' + color_uppercase">{{ color_uppercase }}</div>
                <div class="pc-color-values">
                    <div class="pc-color-value-row" role="button" tabindex="0" @click="copyToClipboard('rgb(' + color_rgb_array['R'] + ', ' + color_rgb_array['G'] + ', ' + color_rgb_array['B'] + ')')" @keydown.enter="copyToClipboard('rgb(' + color_rgb_array['R'] + ', ' + color_rgb_array['G'] + ', ' + color_rgb_array['B'] + ')')" title="点击复制 RGB">
                        <span class="pc-color-label">RGB</span>
                        <span>{{ color_rgb_array['R'] }} , {{ color_rgb_array['G'] }} , {{ color_rgb_array['B'] }}</span>
                    </div>
                    <div class="pc-color-value-row" role="button" tabindex="0" @click="copyToClipboard(color_uppercase)" @keydown.enter="copyToClipboard(color_uppercase)" title="点击复制 HEX">
            ...
            <button class="pc-fab" onclick="randomcolor()" title="随机颜色" aria-label="随机颜色">
            ...
            <div class="pc-toast" v-if="showToast" role="status" aria-live="polite">
        ...
        <div id="content">
            <?php if (isset($sqlres)) {
                ...
                <div class="palette-info palette-info-material">
                    <div class="palette-info-title"><?php echo htmlspecialchars($data['title'], ENT_QUOTES, 'UTF-8');?></div>
                    <div class="palette-info-author">By <?php echo htmlspecialchars($data['author'], ENT_QUOTES, 'UTF-8');?></div>
                    <div class="palette-info-description"><?php echo nl2br(htmlspecialchars($data['description'], ENT_QUOTES, 'UTF-8'));?></div>
                </div>
            <?php }
                } ?>
            <div v-for="group in list" class="color-group color-group-material" v-if="notfound == false">
                ...
                <div class="color-group-container">
                    <div v-for="item in sorted(group.colors , group.autosort)" class="color-item color-item-material" v-bind:style="'--item-color: ' + item.hex + '; --item-color-rgb: ' + colorhex2str(item.hex) + '; --item-shadow-color-rgb: ' + colorhex2str(item.hex) + ';'" v-bind:hex="item.hex" v-bind:class="{current: color == item.hex , toolight: vistoolight(item.hex) , 'color-group-subtitle': (item.hex == 'subtitle')}">
                        <div v-if="item.hex != 'subtitle'" v-bind:style="{background: item.hex}" class="color-preview color-preview-material" v-on:click="vchangecolor(item.hex);" role="button" tabindex="0" @keydown.enter="vchangecolor(item.hex)" :title="item.hex"></div>
